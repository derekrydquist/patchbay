...
- **Instrument removal now has a hover "−" button, matching section rows** — Previously, removing an instrument was only reachable via right-click → "Remove Instrument" on the column header; sections already had a hover-reveal "−" button next to the row label with no equivalent for instruments. Added a matching hover-reveal "−" button to the instrument column header (`group/instrument` pattern, mirrors the section row's `group/section` button exactly), calling the same `setRemoveTrackId` state and existing confirmation dialog — no changes to the underlying `deleteTrackMutation` or dialog. The right-click path remains as a redundant trigger, same as sections. Also added `min-w-0` to the instrument name's flex context so long names still truncate correctly now that the header has a second flex child.
