...
| `loose_files` | Song-scoped files uploaded without a Track/Section destination, staged for organizing via drag. No `isFinal`/`active`/`sectionName` — those concepts don't apply until a file is placed. `POST /api/loose-files/:id/organize` and `POST /api/loose-files/:id/place-on-timeline` are the only two write paths that turn a loose file into a real `clips` row (and, for the latter, also a `timeline_clips` row) — both go through a single shared `materializeLooseFile`-family function in `storage.ts` so a loose file can never be duplicated or left half-migrated. |
