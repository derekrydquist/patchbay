...
| Tab URL persistence (SongHome) | ✅ Done | Active tab (`Overview` / `Review`) is synced to `?tab=review` in the URL (Overview omits the param for a clean URL). `activeTab`, `autoReviewId`, and `autoCommentId` are derived from `useSearch()` (wouter) on every render — reactive to URL changes without a remount. The tab button only calls `setLocation`; no separate `setActiveTab` state exists. |
