...
|---|---|---|
| Dashboard page | ✅ Done | Two-column grid layout: left column (2/3) has Your Songs list + Upcoming Tasks, right column (1/3) is a fixed-height scrollable Activity sidebar; sidebar height measured once on mount via `getBoundingClientRect()` after songs load — never updates when task list expands; right column has `pt-1.5` to compensate for font-metric half-leading difference between the `text-2xl font-heading` left heading and the `text-xs` Activity label, keeping both visually top-aligned; stacks vertically on mobile; song cards styled as gradient banners (gold chevron, BPM badge); cross-song task list filtered to current user sorted by due date; activity feed polls every 10s with all events in the scrollable sidebar |
