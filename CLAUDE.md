...
| `loose_files` | Files uploaded without an organized destination. `songId` is nullable — NOT NULL means the file belongs to that song (real or idea-type), sitting unorganized alongside its content; NULL means band-wide/unassigned, scoped by `bandId` instead, sitting at the top of the Ideas shelf until dragged onto an Idea. `trackId` is also nullable and adds a third resting tier: NOT NULL means the file is **Track-scoped** — its Track is known but it has no `sectionName` yet — and it renders only in that Track's own Sections column (below the Section list) in MediaBucket, not the Tracks column. Reachable three ways, all converging on `POST /api/loose-files/:id/assign-track` (a plain field update — unlike organize/place-on-timeline below, this never materializes a `clips` row): (1) drag a Tracks-column file onto a Track row, (2) drag one directly into the Sections column's open background, (3) drag a Track-scoped file from one track's Sections column onto a *different* track's row — deliberate cross-track reassignment, not an accident. Dragging onto a Section row still organizes into a real `clips` row via the route below, unchanged. No `isFinal`/`active`/`sectionName` — those concepts don't apply until a file is organized. `POST /api/loose-files/:id/organize` and `POST /api/loose-files/:id/place-on-timeline` are the only two write paths that turn a loose file into a real `clips` row (and, for the latter, a `timeline_clips` row too) — both go through a single shared `materializeLooseFile`-family function in `storage.ts`, so a loose file can never be duplicated or left half-migrated; `materializeLooseFileCore` resolves its destination entirely from its own `trackId`/`sectionName` parameters, never from the loose file's own `trackId` column, so a Track-scoped file organized into a Section works with no special-casing. `GET/POST /api/loose-files/unassigned` handle the band-wide tier specifically; `GET /api/tracks/:trackId/loose-files` backs the Track-scoped tier. **Deletable via right-click "Delete"** on every surface that renders one (Songs quick-browser, Ideas shelf — both the Column 1 list and the one-off preview card, SongHome, Workspace/MediaBucket) — one shared `LooseFileDeleteMenu` component (`client/src/components/daw/LooseFileRow.tsx`) drives all of them via `DELETE /api/loose-files/:id`, no confirmation dialog. `storage.deleteLooseFile` also unlinks the physical file from `uploads/`, but only after confirming no other `loose_files`/`clips`/`timeline_clips` row still has that same `src` — this guard exists specifically because of the upload-collision bug documented under "File Uploads" below, which currently leaves many loose files sharing one physical file on disk; deleting one row must never take that shared file out from under the others. |
...
| `global_settings` | Single-row config table (PK = `'global'`). Stores `defaultInstruments` (JSON string[]), `defaultSections` (JSON string[]), and `defaultBpm` (integer). `getSettings()` auto-inserts the factory row on first read so the row always exists. Used by the New Project modal to pre-populate tracks/sections/BPM. |
...
**Naming note:** the `instrument_tracks` table, its `instrument` column/field references, and internal identifiers (`instrumentForUpload`, `group/instrument`, `AddInstrumentModal`, etc.) are unchanged. Only the **user-facing label** was renamed, app-wide, from "Instrument"/"Instruments" to "Track"/"Tracks" (client-side UI copy across MediaBucket, Track.tsx, AddInstrumentModal, AddSectionModal, ProductionTracker, Dashboard, and AppHeader; server-side display strings in `routes.ts`'s duplicate-name/error messages and Activity Feed descriptions). See "Recently fixed bugs" below.

**Loose-file Activity Feed wording (idea-type songs):** loose-file upload/organize descriptions branch on the parent song's `type` to avoid the redundant "...to/into Files — Files" wording every idea-type song used to produce (every idea resolves to the same single hidden default track/section, both literally named "Files"). Real songs are unchanged. See "Loose-file events" in `.claude/skills/activity-feed/SKILL.md` for the exact wording per event type.
...
**`songs.sections` is written once at song creation and never updated afterward** — `POST /api/songs/:songId/sections` and section hide/restore both operate on `ideas` rows only. Do not treat `songs.sections` as live section state for any feature; derive the current section list from active `ideas` rows instead (the union of `sectionName` across active tracks/ideas is the live equivalent). Discovered while building the loose-file "Place on Timeline" picker, whose Section dropdown originally would have silently gone stale using this column.
...
**Known bug — loose-file uploads collide on one physical filename per extension.** `UploadModal` always calls `uploadFile(file, 'loose', 'unplaced', '')` for both `'loose'` and `'band-loose'` mode (no `ideaId`), so `POST /api/upload`'s `existingCount = ideaId ? countClipsForIdea(ideaId) : 0` is always `0` and every loose upload computes version 1 — every loose-file upload sharing an extension silently overwrites the previous one's physical bytes on disk (`loose_unplaced_v1.mp3`, `loose_unplaced_v1.wav`, etc.), even though each keeps its own distinct `loose_files` DB row pointing at that now-shared `src`. Confirmed live in this project's data: one physical file had 24+ referencing rows across `loose_files`, `clips`, and `timeline_clips` (a `clips`/`timeline_clips` row ends up sharing it too once a colliding loose file is organized/placed, since `materializeLooseFileCore` copies `src` as-is without renaming). Found and left unfixed during the loose-file delete rollout (see `loose_files` above and "Recently fixed bugs") — that work added a reference-count guard so deleting one row can no longer take the shared file out from under every other row still pointing at it, but the root cause (loose uploads never getting a unique physical filename) is still present. A real fix belongs in `POST /api/upload` / `UploadModal`'s loose-mode call, not in the delete path — not scoped or started.
...
interface UploadModalProps {
  ...
  songId?: string;                // required for 'placed'/'loose'; omitted for 'band-loose'
  ...
  mode?: 'placed' | 'loose' | 'band-loose'; // default: 'placed'
  onUploadSuccess?: (result: { destTrackId: string; destIdeaId: string }) => void;
}
```

**`mode`** — `'placed'` (default) resolves a real destination and creates a `clips` row, same as always. `'loose'` skips destination resolution entirely — files land in `loose_files` scoped to `songId`, to be organized later via drag (see `loose_files` in the Tables list above). `'band-loose'` also skips destination resolution but has no `songId` at all — files land in `loose_files` with `songId: null`, scoped to the session's band instead; never pass `songId` alongside this mode. `'band-loose'` is the only mode the Ideas shelf's Column 1 "Upload Files" uses — it must work with no Idea selected, so it can't depend on a destination existing at all.

**Destination dropdown** — shown only in `'placed'` mode, at the top of the modal in a consistent position, visible before any files are added. Options format: `"{Track} → {Section}"` (e.g. `"Guitar 2 → Chorus 2"`). The selected value also displays in this format. When `defaultIdeaId` is provided, the dropdown pre-selects the matching option but the user can change it. When `defaultIdeaId` is not provided, it shows "Select Destination" placeholder. `'loose'`/`'band-loose'` show neither the dropdown nor a fixed "Uploading to" box — just a note that files are added without a destination.
...
**Used in:**
- `MediaBucket.tsx` — opened by the header Upload button and by drag-drop onto the versions column or section rows; also reused, unmodified, by the SongHome Song Files tab and the Workspace Arrangement tab (both just mount `MediaBucket`)
- `Dashboard.tsx` — opened by the FILES column Upload button and by drag-drop onto the FILES column (Songs quick-browser, `'placed'` mode) or the Ideas shelf's Files column (`'placed'`, destination always the Idea's single auto-created Folder — see "Idea-type songs" under Database above); also Column 1's "Upload Files" (`'band-loose'`) and, on real songs, "Add Files" (`'loose'`)
...
- **Any mutation that changes loose-file state must invalidate `bucketKeys.bucket`, not just the more specific `looseFileKeys.*` keys, whenever anything cached on the bucket response depends on it.** The bucket response (`GET /api/songs/:id/bucket`) now carries computed per-track fields like `hasLooseFiles` (drives the Tracks-column folder-fill icon) — a mutation that invalidates only `looseFileKeys.list`/`byTrack`/`unassigned` leaves that computed field stale until a manual reload, even though the more specific lists refresh correctly. This was missed on both `useDeleteLooseFile` and `useAssignLooseFileTrack` and needed two separate follow-up fixes — when adding a new loose-file mutation, invalidate `bucketKeys.bucket(songId)` by default rather than assuming the narrower keys are sufficient.
...
| Lyrics tab architecture (autosave textarea, select-to-comment anchoring, replies/resolved/@mentions, composer positioning, browser-timing gotchas) | `.claude/skills/lyrics/SKILL.md` | Working on the Lyrics tab |
...
- **Duplicate album names:** rejected client-side case-insensitively in `handleCreateAlbum` /
  `handleRenameAlbum`; the API itself allows them (consistent with sections/tracks convention).
...
- **Track name uniqueness** — tracks can no longer share a name within a song, active or hidden. Full `UNIQUE(songId, name)` index (not partial); 409 checks and client-side guards updated across MediaBucket, Production Tracker, Dashboard. Error message points to the restore option when the conflicting name is hidden.
...
- **Section restore modal not closing** — Production Tracker's `restoreSectionMutation.onSuccess` was missing the modal-close state resets (`setIsAddSectionOpen(false)`, etc.) that the track-restore counterpart already had. Fixed to match.
...
- **Track removal now has a hover "−" button, matching section rows** — Previously, removing a track was only reachable via right-click → "Remove Track" on the column header; sections already had a hover-reveal "−" button next to the row label with no equivalent for tracks. Added a matching hover-reveal "−" button to the track column header (`group/instrument` pattern in the code, mirrors the section row's `group/section` button exactly), calling the same `setRemoveTrackId` state and existing confirmation dialog — no changes to the underlying `deleteTrackMutation` or dialog. The right-click path remains as a redundant trigger, same as sections. Also added `min-w-0` to the track name's flex context so long names still truncate correctly now that the header has a second flex child.
- **`deleteTrackMutation` (track removal) had no error handling — failed removals were silent** — The confirmation dialog would simply not close and the track wouldn't disappear, with no feedback telling the user why. `useDeleteTrack`'s hook already supported an `onError` callback; the call site in `ProductionTracker.tsx` just never passed one. Added `onError` showing a destructive toast ("Failed to remove track" + error message), mirroring the pattern already used by `hideSectionMutation`. Verified via real-browser test (dev server stopped, removal attempted, toast confirmed, track remained visible).
...
- **Lyrics reload race condition** (textarea placeholder flash, comments sidebar empty/wrong-order flash) — see "Reload race condition (fixed)" in `.claude/skills/lyrics/SKILL.md` for full detail.
- **Loose-file uploads (song-scoped, unplaced files) — shipped end to end.** New `loose_files` table backs a destination-free upload entry point (a `mode="loose"` on `UploadModal` skips the destination picker entirely) for files not yet assigned to a Track/Section. Materializing a loose file into a real `clips` row — and, when placed directly on the timeline, also a `timeline_clips` row — runs through a single shared transactional function in `storage.ts` (the `materializeLooseFile` family), so a loose file can never end up duplicated or half-migrated across `POST /api/loose-files/:id/organize` and `POST /api/loose-files/:id/place-on-timeline`. Loose files render alongside Track folders in MediaBucket's Tracks column and in the Ideas-shelf Folders column (Dashboard.tsx), draggable onto a Section/Versions column (organize) or onto the Timeline (place-on-timeline).
- **MediaBucket's Versions column wasn't a valid loose-file organize target** — only the Sections column accepted the drop; Versions used a separate native-HTML5-drag mechanism (for OS file drops from Finder) with no dnd-kit `useDroppable` registered at all, so a dnd-kit-driven loose-file drag could never resolve a collision there. Added one (`bucket-versions||{ideaId}`, same `{trackId, sectionName}` data shape as the Section row), and generalized `trackFirstCollision`'s precise X+Y bounds pass — previously scoped only to `bucket-section||` — to also cover the new prefix. Both are narrow side-column rows sharing a Y-band with other columns, not full-width bands, so Timeline's existing vertical-band-only heuristic (correct for full-width track rows) would otherwise match whichever row happened to share that Y position regardless of X.
- **Loose-file "Place on Timeline" replaced a confirm-only modal with a cascading Track → Section picker** — the original modal silently dead-ended (a destructive toast, no path forward) whenever the drop's X position didn't resolve to an existing section: an empty timeline, or a drop past/before every rendered section. It now always opens a modal with Track/Section `Select` dropdowns (same interaction shape as Dashboard's "Add to Song" cascade — a separate component/data source, not shared), pre-filled from the drop point where resolvable and left for manual selection otherwise. Section options are the live union of `sectionName` across active `ideas` on active tracks — not `songs.sections` (see the caution under Database above) and not `sectionLayout` (clip-derived, the actual cause of the original dead end).
- **Band-wide, unassigned loose files** — `loose_files.songId` made nullable (plus a new `bandId` column), migrated via SQLite's rebuild-and-swap pattern (`ALTER COLUMN` isn't supported; guarded by a `pragma_table_info` check in `server/db.ts` so it only runs once, same idiom as every other schema change in that file). Lets a file be uploaded with no song/Idea association at all, scoped only to the band — powers the Ideas shelf Column 1 "Upload Files" entry point. New `GET/POST /api/loose-files/unassigned` routes and an `assertLooseFileOwned` helper (chain-walks through `songId` when set, falls back to the loose file's own `bandId` when not) cover ownership for both tiers across the existing organize/place-on-timeline/delete routes.
- **Loose-file drag-to-organize consolidated into one shared hook** — `useLooseFileOrganizeDnd()` (`client/src/hooks/use-loose-file-organize-dnd.ts`) plus `LooseFileDragOverlay` now back every surface that supports it (Workspace/MediaBucket via Timeline's `DndContext`, the Dashboard Ideas shelf, the Dashboard Songs quick-browser, and the SongHome Song Files tab), replacing three independently hand-built implementations. Two real bugs were found and fixed along the way: (a) MediaBucket's and Dashboard Songs-quick-browser's Versions/Files-column droppables were deriving their dnd-kit `id` from `selectedIdea.id` (or equivalent) — dnd-kit tears down and re-registers a droppable whenever its `id` changes, so switching sections could leave the drop target with no measured rect on record and silently swallow the next drop; fixed by giving both a fixed, stable id string instead (destination resolution already read `over.data.current`, never `over.id`, so the id itself carries no meaning beyond being a stable registry key). (b) The Songs quick-browser's drag ghost rendered via `LooseFileRow`'s own inline transform, which visually clipped/detached inside its scrolling, `overflow-hidden` column; fixed by switching to a portal-based `DragOverlay` (`LooseFileDragOverlay`), matching every other drag ghost in the app.
- **SongHome's "Song Files" tab never actually supported loose-file drag-to-organize** — its `<DndContext>` had no `sensors`/`collisionDetection`/`onDragEnd` wired at all (a bare `<DndContext>` with no props); MediaBucket's own droppables existed and rendered fine, but nothing was listening for a drag to land on them, so drops there silently no-opped. Fixed by wiring the same shared `useLooseFileOrganizeDnd()` hook every other surface uses.
- **Ideas shelf flattened from 3 levels to 2 — the Folder layer removed entirely.** See the updated "Idea Shelf" entry in "On the horizon" below for the full before/after and mechanism.
- **Clicking a loose file now loads a preview; selection highlight made mutually exclusive.** See the same "Idea Shelf" entry below.
- **Small polish pass on top of loose-files/Ideas-shelf work** — three items: (a) the Ideas shelf's Column 1 rows switched from a lightbulb icon to the Folder icon already used by Instruments/Track rows elsewhere, for visual consistency; (b) the user-facing "Instrument"/"Instruments" label was renamed to "Track"/"Tracks" everywhere in the file-organizer UI — client-side copy across MediaBucket, Track.tsx, AddInstrumentModal, AddSectionModal, ProductionTracker, Dashboard, and AppHeader, plus server-side display strings (`routes.ts` duplicate-name/error messages and Activity Feed descriptions) — see the naming note under Database above; (c) loose-file Activity Feed wording for idea-type songs no longer shows the redundant "added/organized X to/into Files — Files" — see "Loose-file Activity Feed wording" under Database above.
- **Loose files had no delete mechanism anywhere in the app** (confirmed via a dedicated Remove/Delete recon: `DELETE /api/loose-files/:id` existed and worked server-side, but the single shared `LooseFileRow` component — used by all four surfaces that render a loose file — had no context menu, no right-click handler, no delete affordance of any kind). Fixed by adding a `LooseFileDeleteMenu` component (`client/src/components/daw/LooseFileRow.tsx`, wraps `useDeleteLooseFile` from `use-bucket-mutations.ts`) that `LooseFileRow` now uses internally — right-click → "Delete", no confirmation, matching the "no confirmation" convention already established elsewhere in this codebase (MediaBucket's "Remove Track"/"Remove Section"). This alone covers three of the four surfaces (Songs quick-browser, SongHome, Workspace/MediaBucket) plus the Ideas shelf's Column 1 list. The fourth spot — the Ideas shelf's one-off loose-file **preview** card (Column 2, "Preview — drag onto an Idea to file it") — renders via a bare `WaveformPlayerCard`, not a `LooseFileRow`, so it needed the menu added separately; found and fixed in a follow-up pass (right-clicking it fell through to the browser's native context menu until then). See `loose_files` in the Database table above.
- **`storage.deleteLooseFile` only removed the DB row — the physical file in `uploads/` was left orphaned on every delete.** Fixed by adding an `fs.unlink` call, but only after discovering (via live data, not theory) that this is unsafe to do unconditionally: `UploadModal` always uploads loose files with a fixed `instrument='loose'/section='unplaced'`/no `ideaId`, so `POST /api/upload` always computes version 1 for a loose upload — every loose-file upload sharing an extension collides on one physical filename and silently overwrites the previous one's bytes on disk, while each keeps its own distinct `loose_files` row. Confirmed live: one physical file (`/uploads/loose_unplaced_v1.mp3`) had 24+ rows referencing it across `loose_files`, `clips`, and `timeline_clips` (materializing a loose file copies its `src` as-is, so the collision can propagate into organized clips too). `deleteLooseFile` now checks for any other `loose_files`/`clips`/`timeline_clips` row sharing the same `src` before unlinking, and skips the unlink if one exists. Verified against live data, not just reasoned about: deleted 3 of 4 rows sharing one collided file (file survived each time, correctly detected the remaining references), then deleted the last real reference to a distinct test file and confirmed via direct filesystem check (not just the DB row) that the file was actually removed; separately verified a deletion in the large `loose_unplaced_v1.mp3` cluster left the file byte-for-byte identical (`cmp`/MD5) and every other referencing row (including an already-organized `clips` row) still resolving and serving correctly afterward. The root upload-side collision bug itself is unfixed — see "File Uploads" above.
- **The Ideas-shelf loose-file preview and organized-clip context menus consolidated into one `IdeaFileContextMenu` component**, giving the preview parity with organized files (Add to Song, Promote to Song, Delete) instead of a bare "Delete"-only menu. See the "Idea Shelf" entry in "On the horizon" below ("Further update, Sept 2026") for the full menu-item-by-menu-item behavior and reasoning, including why More Info/Add Note are disabled for a loose file and how Add to Song/Promote to Song were made to work identically via `looseFileAsApiClip()`.
- **Track-scoped loose-file tier added — a third resting state alongside band-wide and song-scoped.** New nullable `loose_files.trackId` column (simple `ALTER TABLE` addition, not a rebuild-and-swap — see `loose_files` above for the full shape). New `TrackFolderRow` component in `MediaBucket.tsx` (extracted the same way `SectionFolderRow` already was, so `useDroppable` is called once per row) registers a `bucket-track||{trackId}` organize-family drop target; `storage.getLooseFilesBySong` now filters `trackId IS NULL` so a Track-scoped file stops appearing in the Tracks column the moment it's assigned. `POST /api/loose-files/:id/assign-track` is a plain field update — deliberately does not go through `materializeLooseFileCore`, since nothing is being organized yet.
- **Track row showed a "can accept drop" highlight for a same-track re-drop that was actually a no-op, and gave no feedback either way.** Root cause: `LooseFileRow`'s drag payload dropped the file's own `trackId` entirely (`looseFileToClip`'s `Clip` shape has no such field), so `TrackFolderRow` had no way to tell "a plain Tracks-column file" from "a file already scoped to this exact track being dragged back onto its own row" — every drag looked identical to the highlight logic. Fixed by carrying `trackId` alongside (not inside) the drag payload's `clip` field, and disabling `TrackFolderRow`'s own droppable (`disabled: isSameTrackDrag`) when the active drag's `trackId` already equals that row's `track.id`, making the same-track case structurally unreachable rather than a handled no-op. A related, separately-discovered bug: even with `disabled` correctly blocking the drop, the highlight still appeared, because `isOver` doesn't check `disabled` — see "Platform and browser gotchas" in `client/src/components/daw/CLAUDE.md`. `useAssignLooseFileTrack` was also extended to accept an `originTrackId` and invalidate the *origin* track's `byTrack` query on a genuine cross-track move, not just the destination's — otherwise the Sections column the user just dragged out of kept showing the file from stale cache.
- **The Sections column's open background (not a specific Section row) is now a valid drop target, routing to the same assign-track destination as a Track-row drop.** New `bucket-sections-bg||` drop-target prefix, scoped to whichever Track is currently selected (same `{trackId}` data shape as the Track-row target, same stable-suffix-id fix `bucketVersionsDropId` already established — never derived from `selectedTrack.id`, to avoid dnd-kit tearing down and re-registering the droppable on every track switch). Required a real fix to `matchOrganizeDropTarget`'s collision priority — see "Platform and browser gotchas" in `client/src/components/daw/CLAUDE.md` for the mechanism (this is the first organize drop-target pair that geometrically nests one inside the other).
- **Track row's folder-fill icon didn't account for Track-scoped loose files — a track with only loose files, no organized clips, showed the outline icon.** Root cause: the icon's `hasFiles` check only looked at `track.ideas[].clips.length`, blind to `loose_files` entirely. Fixed by computing `hasLooseFiles` server-side in `storage.getBucket()` (one extra query, mirroring how `hasNew` is already computed per-idea in the same endpoint) and returning it on `ApiTrack`; `TrackFolderRow`'s check is now `ideas.some(clips.length > 0) || track.hasLooseFiles`. Needed two follow-up cache-invalidation fixes once shipped — `useDeleteLooseFile` and `useAssignLooseFileTrack` both invalidated the narrower `looseFileKeys.*` keys but never `bucketKeys.bucket`, so the icon only updated after a manual reload; see the query-invalidation gotcha under "What To Avoid".
...
- **Manual drag-to-reorder (tracks + sections)** — track `sortOrder` data is now clean and ready; sections `sortOrder` is now clean too (fixed by section-add redesign).
- **Section row-label right-click trigger has no visual affordance** — Production Tracker's sticky left column section labels are right-clickable ("Remove Section") but there's no hover indicator, unlike track column headers' hover-reveal `+`. Consider a tooltip or hover-reveal `···` button.
...
- ~~**Idea Shelf: instrument-assigned but not song-assigned ideas**~~ **Resolved, then flattened further (Sept 2026).** The Library "Ideas" tab was originally built as a three-level hierarchy — Ideas → Parts (Folders) → Files — as a UI-level repurposing of the same Song/InstrumentFolder/Idea tables real songs use. It is now a **flat two-level structure: Idea → Files directly, with no Folder/Part-selection step in the UI at all.** "Ideas" are still `songs` rows with `type = 'idea'` (`Dashboard.tsx` filters `songs.filter(s => s.type === 'idea')`). Under the hood, each idea-type song still has exactly one `instrument_tracks` row + one paired `ideas` row — but it's now auto-created and never surfaced, via `storage.ensureIdeaDefaultFolder(songId)` (called at idea creation, defensively on every bucket fetch, and from the songs-list route), rather than user-created via an "Add Folder" action. So the underlying clip/idea data model didn't need to change — only the UI's navigation depth did; `addPartMutation` and the "Add Folder" modal were deleted outright, not just hidden. `GET /api/songs` now also returns `defaultTrackId`/`defaultSectionName` per idea-type song, so a Column-1 Idea row can itself be a drag-and-drop organize target for a loose file without first being opened. Column 2 (Files) renders the flattened union of clips across every track/idea the song has — normally exactly one, but this stays correct even for a hypothetical legacy idea that ended up with more than one (checked before building: none currently do). **The known follow-up risk from the old three-level design is now moot** — `instrumentTracks.name`/`ideas.sectionName` drift on rename can't happen because there's no UI to rename the (invisible, fixed-named `"Files"`) Folder at all. What's still true from before: the right-click context menu on a shelf file (More Info, Add Note, Add to Song, Promote to Song, no Mark as Final) is unchanged; `Add to Song` still fully copies the clip (new `clips` row, re-uploaded file, `isFinal: false` always, no live link back); the `clips.addedToSongs` "belongs to song X" badge is still a denormalized snapshot, not a live join, and can go stale on a rename elsewhere. New in the flattened version: a loose file — band-wide/unassigned (visible in the Ideas list itself, Column 1) or scoped to one Idea (visible in that Idea's flat file list) — can be dragged onto an Idea to organize it in, and **clicking a loose file loads a one-off preview into the Files column** (a new interaction with no prior precedent elsewhere in the app; Workspace/Songs quick-browser only ever preview already-placed clips), taking selection-highlight priority over whatever Idea was previously selected until dismissed or a different Idea is chosen. **Further update (loose-file delete rollout, Sept 2026):** the right-click context menu described above is now a single shared `IdeaFileContextMenu` component (`Dashboard.tsx`) used by both the organized-clip cards (`clips.map`) and the loose-file preview card — previously the preview had no context menu at all (a bare `WaveformPlayerCard`, fixed in the same rollout after being found missing entirely). The dead `{selectedFile?.type !== 'idea' && <Mark as Final>}` branch (this menu only ever renders inside the Ideas-shelf's idea-type flow, so the condition was always false) was dropped during the extraction — no behavior change. Menu items now behave per-source: **More Info and Add Note are disabled for a loose file** — both need a real `clips` row (`clip_comments.clipId` is a hard FK to `clips.id`; the Musical Intelligence fields save via `PATCH /api/clips/:clipId`), which a loose file doesn't have yet. **Add to Song and Promote to Song work identically for a loose file** via a `looseFileAsApiClip()` adapter — see "Add to Song" / "Promote to Song" in `.claude/skills/activity-feed/SKILL.md` for the full mechanism, the loose-file adapter, and why the source-update PATCH is skipped for a loose source. And the preview card now also carries **Delete** (`DELETE /api/loose-files/:id`, no confirmation) as a trailing menu item — see `loose_files` in the Database table above and "Recently fixed bugs" below.
...
- **Band-wide loose-file upload has no Activity Feed entry** — `activity_log.songId` is `NOT NULL`, and an unassigned loose file has no song yet, so there's nowhere to log it until it's organized onto an Idea (which does log correctly today). Fix would need `songId` nullable + a `bandId` column on `activity_log`, plus a UI decision for where an unscoped entry renders (likely Dashboard's global feed). Schema change is low-risk; the UI placement decision is the real work. Not committed to — revisit if the visibility gap becomes a real problem in practice.
- **More Info and Add Note for loose files** — currently disabled by design in `IdeaFileContextMenu` (see the Idea Shelf entry above), not yet built. Derek wants a way to leave notes on a file before it's ever organized — e.g. "this riff rules but I don't know where to file it yet." Scoped but not started: **More Info** is likely small — loose files already store a `ClipMetadata`-shaped `metadata` field, so a read-only info view may just need to read that instead of assuming a `clips` row. **Add Note** needs more: there is no existing table to store a comment against a `loose_files` row (`clip_comments.clipId` is a hard FK to `clips.id`, not reusable as-is), so this needs either a new table or a schema change to `clip_comments`, plus a product decision on what happens to a loose file's notes if/when it's later organized into a real clip (migrate them to `clip_comments` under the new `clips.id`? leave them behind?).
- **Organized-clip deletion (a real `clips` row) remains out of scope** — the loose-file delete rollout (see `loose_files` above and "Recently fixed bugs") deliberately did not touch deleting an already-organized clip. That's item-4 territory from the earlier paused Remove/Delete audit: `BucketClip`'s "Remove" (`PATCH /api/clips/:clipId { active: false }`) still has no confirmation dialog regardless of `isFinal`, and still doesn't reconcile `isFinal` on removal (a removed final clip leaves stale `isFinal: true` on any placed `timeline_clips` instances and stale "Complete" task status behind) — both confirmed still-open bugs, not fixed by this or any session yet. Deliberately deferred to its own future session rather than bundled into loose-file work.
